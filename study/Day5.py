...
def describe_pet(pet_name,animal_type='cat'):#默认参数跟随在非默认参数后面，等号两边一般不要有空格
    print("\nI have a "+animal_type+".")
    print("My "+animal_type+"'s name is "+pet_name.title()+".")
...
describe_pet(animal_type='hamster',pet_name='harry')#等号两边一般不要有空格
...
from pizza import make_pizza,show#导入模块中的特定函数，可以多个
make_pizza(12,'pepperoni')
make_pizza(15,'pepperoni','mushrooms','cheese')
...
# from pizza import make_pizza, show as mp, sw 不能同时给两个函数别名
# mp(12,'pepperoni')

import pizza as p #模块别名
p.show()

from pizza import * #导入模块的所有函数，与导入模块的区别是使用函数时不用加上模块名
show()              #最好不要用，可能函数重名，不容易区分
